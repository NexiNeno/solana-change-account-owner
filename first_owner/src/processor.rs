...
impl Processor {
    pub fn process(
        ...
        let instruction = FirstOwnerInstruction::unpack(instruction_data)?;

        match instruction {
            FirstOwnerInstruction::Increase {} => {
                Self::process_increase(accounts)?;
            }
            FirstOwnerInstruction::ChangeOwnership {} => {
                Self::process_change_ownership(accounts, program_id)?;
            }
            FirstOwnerInstruction::ZeroOutAccount {} => {
    ...
    pub fn process_change_ownership(
        ...
        if counter.owner != program_id {
            return Err(FirstOwnerError::WrongAccountOwner.into());
        }
